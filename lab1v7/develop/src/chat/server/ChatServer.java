    ...
    Runnable			// for the distribution thread.
{
   ...
   */
  protected LinkedList<String> msgClients =
   ...
   * the message queue, builds two ChatNotification events(one for
   * message sender, one for others) and sends them to clients that
   * has registered a remote event listener with us separately.
   * We dequeues the msgClients queue to identify message sender.
  ...
  public void run () {
    ...
    while (runDelivery) {
      ...
      if (msg != null) {
	      ...
	      ChatNotification note = new ChatNotification (this, msg, msgCount,0);
        ChatNotification note0 = new ChatNotification (this, msg, msgCount,1);
