    ...
    RemoteEventListener		// So we can receive chat notifications
{
  ...
  /**
  * LinkedList for recording beginTime when message sent.
  */
  protected LinkedList<Long>time=new LinkedList<Long>();
   ...
   * method to notify us of a new chat message. If the message is sent
   * by us, notify us that message sent successfully and display total
   * time.
  ...
  public void connectToChat (String serverNamePattern)
  {
	...
	for (String svname : nsmap.keySet()) {
      ...
      if (selectedSit == null) {
	      System.out.printf("No servers found matching '%s'%n", serverNamePattern);
      }
      else {
