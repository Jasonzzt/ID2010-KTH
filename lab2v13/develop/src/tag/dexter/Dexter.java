...
public class Dexter implements Serializable {
   ...
   *
   */
  private BailiffInterface preBailiff = null;
  ...
  public void setTagged(boolean isTagged_) {
    isTagged = isTagged_;
  }
   ...
   * lookup server.
   */
  private long retrySleep = 5 * 1000; // 5 seconds
  ...
  public void topLevel()
    ...
    for (; ; ) {
      ...
      while (0 < nofItems) {
        ...
        int idx = 0;
        boolean enter;
        //chose the bailiff without 'it'
        do{
          idx = rnd.nextInt(nofItems);
          Object obj = svcItems[idx].service;
          enter=false;
        ...
        if (obj instanceof BailiffInterface) {
          ...
          if(preBailiff != null) {
            debugMsg(preBailiff.getBID());
            debugMsg("preBailiff");
            List<Dexter> list = preBailiff.getDexters();
            for(int i=0; i< list.size(); i++)
            {
              //solve 'it' disappearance problem
              if(list.get(i).getID().equals(getID()))
              {
                setTagged(list.get(i).isTagged());
                break;
              }
            }
            preBailiff.remove(this);// remove this before termination
          }
          debugMsg("preBailiff1111");
          try {
            String response = bfi.ping(this); // Ping it
            debugMsg(response);
            accepted = true;  // It worked!
            preBailiff = bfi; //saving for future ping - to remove dexter from list
            debugMsg("it:"+bfi.getIt());
            if(isTagged)
            {
              List<Dexter> list = bfi.getDexters();
              for(int i=0; i<list.size(); i++)
