...
//  List<String> list = new ArrayList<String>();
  public String ping (Dexter dex) throws java.rmi.RemoteException
  {
    ...
    System.out.println(df.format(new Date())+":  Current size: " + getDexters().size() + " (added "+dex.getID()+" )");
  ...
  {
    //System.out.println("remove "+dex.getID());
    List<Dexter> ld = getDexters();
    ld.removeIf(s -> s.getID().equals(dex.getID()));
    setDexter(ld);
    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    System.out.println(df.format(new Date())+":  Current size: " + getDexters().size() + " (removed "+dex.getID()+" )");
  ...
  public String getIt(){
    ...
    for (int i=0; i<listDexters.size(); i++) {
      if(listDexters.get(i).isTagged()) it=listDexters.get(i).getID();
    }
    return it;
  }
  ...
  public void tag(Dexter oldIt, Dexter newIt)
  {
    List<Dexter> list = listDexters;
    for(int i=0;i<list.size();i++){
      if(list.get(i).getID().equals(newIt.getID())) {
        list.get(i).setTagged(true);
        break;
      }
    }
    for(int i=0;i<list.size();i++){
      if(list.get(i).getID().equals(oldIt.getID())) {
      ...
      java.rmi.RemoteException, NoSuchMethodException
  {
    ...
    List<Dexter> list = getDexters();
    for(int i=0; i<getDexters().size(); i++)
    {
      if(list.get(i).getID().equals(tmp.getID()))
      {
        tmp.setTagged(list.get(i).isTagged());
        break;
      }
    }
    ...
    System.out.println("Current Dexters Here:");
    for (int i=0; i<listDexters.size(); i++) {
      if(listDexters.get(i).isTagged()) System.out.println(listDexters.get(i).getID()+" (it)");
      else System.out.println(listDexters.get(i).getID() + " ");
    }
  }
